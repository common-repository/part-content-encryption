...
{
    wp_enqueue_style('encryption-style', plugins_url('/style.css', __FILE__), array(), '1.1.1', 'all');
}
add_action('wp_enqueue_scripts', 'e_encryption_enqueue_style');

function e_encryption($atts, $content = null) {
    extract(shortcode_atts(array('key' => null), $atts));
    $e_encryption_key = isset($_POST['e_encryption_key']) ? sanitize_text_field($_POST['e_encryption_key']) : null;
    if (!is_null($e_encryption_key) && $e_encryption_key == $key) {
        return '<div class="e-encryption">' . $content . '</div>';
    } else {
        return '
        <form class="e-encryption" action="' . get_permalink() . '" method="post" name="e-encryption">
            <label>输入密码查看加密内容：</label>
            <input type="password" name="e_encryption_key" class="encry-i" maxlength="50">
            <input type="submit" class="encry-s" value="确定">
